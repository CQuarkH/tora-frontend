// emotion_selector_section.dart
import 'package:flutter/material.dart';
import 'package:tora_frontend/features/child/models/emotion_record.dart';
import 'package:tora_frontend/features/child/widgets/emotion_option.dart';

class EmotionSelectorSection extends StatelessWidget {
  final EmotionRecord? currentEmotion;
  final ValueChanged<Emotion> onEmotionSelected;

  const EmotionSelectorSection({
    super.key,
    required this.currentEmotion,
    required this.onEmotionSelected,
  });

  @override
  Widget build(BuildContext context) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(20.0),
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(16.0),
        border: Border.all(color: Colors.grey.shade300),
        boxShadow: [
          BoxShadow(
            color: Colors.grey.withOpacity(0.1),
            blurRadius: 10.0,
            offset: const Offset(0, 2),
          ),
        ],
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text(
            '¿Cómo te sientes?',
            style: TextStyle(
              fontSize: 18.0,
              fontWeight: FontWeight.w600,
              color: Colors.black87,
            ),
          ),
          const SizedBox(height: 8.0),
          Text(
            'Dime cómo te sientes en este momento',
            style: TextStyle(fontSize: 14.0, color: Colors.grey[600]),
          ),
          if (currentEmotion != null) ...[
            const SizedBox(height: 12.0),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
              decoration: BoxDecoration(
                color: currentEmotion!.emotion.backgroundColor,
                borderRadius: BorderRadius.circular(20),
              ),
              child: Text(
                'Registrado: ${currentEmotion!.emotion.displayName} ${currentEmotion!.emotion.emoji}',
                style: const TextStyle(
                  fontSize: 12,
                  fontWeight: FontWeight.w500,
                ),
              ),
            ),
          ],
          const SizedBox(height: 16.0),
          GridView.count(
            shrinkWrap: true,
            physics: const NeverScrollableScrollPhysics(),
            crossAxisCount: 3,
            mainAxisSpacing: 12.0,
            crossAxisSpacing: 12.0,
            childAspectRatio: 0.9, // Reducido para dar más espacio vertical
            children: Emotion.values.map((emotion) {
              final isSelected = currentEmotion?.emotion == emotion;
              return EmotionOption(
                emotion: emotion,
                isSelected: isSelected,
                onTap: () => onEmotionSelected(emotion),
              );
            }).toList(),
          ),
        ],
      ),
    );
  }
}
